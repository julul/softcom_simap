...
#classification_model_args['use_cuda'] = args.use_cuda

#print("use_cuda " + str(args.use_cuda) + "\n")
print("train_batch_size " + str(args.train_batch_size) + "\n")
...
if args.runmode == 'fold1':   # fine-tuning procedure
    results_object={}
    results_object["classifier"] = classifier
    results_object["tune_params"] = modelargs_tuning_grid
    results_object['score'] = score
    file = open(results_path, "w",encoding='utf8')
    file.write(json.dumps(results_object))
    ...
    for key in languages:
        dep_result = compare_lang_dependency(key, best_params) # returns 1 (dependent is better) or 0 (independent is better)   
    ...
    if os.path.isfile(results_path):
        ...
        results_object["runmodel"]["classifier"] = classifier
    ...
    # run 5 times with best parameter values and take the average 
    runmodel_results_averaged = get_averaged_results(classification_model_args, user_params, saveas='runmodel') # apply user params and run num_runs times and take the average of the results as best result
    # save the results
    user_combination = {}
    ...
    ########## test language (in)dependency and save the results
    for key in languages:
        dep_result = compare_lang_dependency(key, user_params) # returns 1 (dependent is better) or 0 (independent is better)   
        ## results already saved in 'compare_lang_dependency' function
        ## OUTPUT the results
        if dep_result == 1:
            print('For the ' + languages[key] + ' language, the language-DEPENDENT classification methodology achieves better classification results according to the ' + score + ' metric\n')
    ...
    testmodel_results = get_results(classification_model_args,best_params1, testing="Julia")
    # save the results  
    best_combination1 = {}
    ...
    file.close()
        ...
        '''
        # load model
        print("InesTest, trainedmodeldir:" + trainedmodeldir[classifier + "_Ines"])
        args_combination['output_dir'] = trainedmodeldir[classifier + "_Ines"]
        model = ClassificationModel(**classification_model_args, args = args_combination)
        
        https://pypi.org/project/simpletransformers/0.48.3/#loading-saved-models
        example: 
        # loading
        ...
        Note that you will need to specify the correct (usually the same used in training) args when loading the mode
        '''
