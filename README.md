...
```
$ python3 tfidf_models.py <classifier> <runmode> [model hyperparameters] --metric --reference 
```
or

```
$ python3 fasttext_model.py <runmode> [model hyperparameters] --metric --reference 
```
or
```
$ python3 bert_model.py <runmode> [model hyperparameters] --metric --reference
```
...
```
After running for a while, try with  `Ctrl + c`, `Ctrl + d`or `Ctrl + \` to stop the process. You can also try to pause with `Ctrl + z` and then continue with `fg`.
...
```
python3 bert_model.py runmodel --train_batch_size=32 --learning_rate=0.00005 --num_train_epochs=4 --max_seq_length=512
```
